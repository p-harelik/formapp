<template>
  <div>
    <v-snackbar
      ...
      color="success"
    >
      Заявка на оформления отпуска № {{result.elementId}} успешно создана!
    ...
    <div class="wrapper-form">
      <form @submit.prevent>
        <p class="subtitle-1 font-weight-medium black--text mt-4 mb-4">Информация по отпуску</p>
        <v-row>
          <v-col
            class="my-0 py-0"
            cols="12"
        ...
        <v-file-input
          v-model="files"
          :label="type === 931 ? 'Заявление (необязательно)' : 'Cправка-вызова с места обучения'"
  ...
  export default {
    ...
    computed: {
      ...
      filesError () {
        ...
        !this.$v.files.required && errors.push('Необходимо прикрепить справку-вызов')
        return errors
      }
    },
    methods: {
      ...mapActions(['vacationRequest']),
      async submit () {
        this.$v.$touch()
        if (this.$v.$invalid) {
          ...
          const result = await this.vacationRequest(formData)
          this.loading = false
          this.result = result
          if (result.elementId) {
            this.successSnackbar = true
            this.description = null
            this.dates = this.files = []
          } else {
